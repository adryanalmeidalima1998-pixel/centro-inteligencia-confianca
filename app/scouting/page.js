...
export default function HomePage() {
  ...
  const k = data?.kpis || {}
  const guarani = data?.guarani?.summary || {}
  return <ScoutingPage maxWidth={1580}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(330px,.75fr)', gap: 14, marginBottom: 14 }} className="scout-two-col">
      <Panel title="Confiança · snapshot competitivo" subtitle={`${guarani.games || 0} partidas coletivas Sportsbase`} accent={C.green} action={<Link href="/elenco" style={{ color: C.green, fontSize: 8.8, fontWeight: 900, textDecoration: 'none' }}>Abrir dossiê →</Link>}>
        {guarani.games ? <><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}><MiniSignal label="Pontos" value={guarani.points || 0} sub={`${num(guarani.performance, 1)}% aproveitamento`} /><MiniSignal label="Saldo" value={`${Number(guarani.goalDifference) > 0 ? '+' : ''}${guarani.goalDifference || 0}`} sub={`${guarani.goalsFor || 0} GP · ${guarani.goalsAgainst || 0} GC`} tone={Number(guarani.goalDifference) >= 0 ? C.green : C.red} /><MiniSignal label="Chances/jogo" value={num(guarani.averages?.chances, 1)} sub={`${num(guarani.averages?.shots, 1)} chutes`} tone={C.blue} /><MiniSignal label="Progressão" value={num(guarani.averages?.progressivePasses, 1)} sub="passes progressivos/jogo" tone={C.purple} /></div><div style={{ marginTop: 12, padding: 12, background: C.green2, borderRadius: 10, border: `1px solid ${C.green3}` }}><p style={{ color: C.green, fontSize: 8.5, fontWeight: 950, textTransform: 'uppercase' }}>Identidade atual</p><strong style={{ display: 'block', color: C.ink, fontSize: 12.5, marginTop: 4 }}>{data?.guarani?.model?.identity || 'Modelo em construção'}</strong></div></> : <EmptyState icon="🟢" title="Confiança sem dados" text="Suba as planilhas coletiva e individual na página Elenco." />}
      </Panel>
      <Panel title="Linha do tempo da posição" subtitle="Rodadas sincronizadas automaticamente; posição confirmada pelo analista quando não existe na fonte"><Timeline items={data?.guarani?.timeline || []} /></Panel>
    </div>
