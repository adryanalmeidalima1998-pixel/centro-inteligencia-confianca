...
function DimensionBar({ label, value }) {
  ...
  return <div style={{marginBottom:7}}>
    ...
    <div style={{height:5,borderRadius:999,background:'#e8f4ec',overflow:'hidden'}}><div style={{height:'100%',width:`${numeric}%`,background:GFC,borderRadius:999}}/></div>
  </div>
}
...
function SourceSwitch({ value, available, onChange }) {
  ...
  return <div style={{display:'flex',gap:5,alignItems:'center',flexWrap:'wrap'}}>
    ...
    {[['auto','Automático'],['sportsbase','Sportsbase'],['wyscout','Wyscout']].filter(([key])=>key==='auto'||available?.[key]).map(([key,label])=><button key={key} onClick={()=>onChange(key)} style={{border:`1px solid ${value===key?GFC:'#dbe7f2'}`,borderRadius:7,padding:'5px 8px',background:value===key?'#eaf4fd':'#fff',color:value===key?GFC:'#64748b',fontSize:9,fontWeight:800,cursor:'pointer'}}>{label}</button>)}
  </div>
}
...
export default function SportsbaseTeamsTable({ slug, sourcePreference='auto', onSourceChange }) {
  ...
  if(source==='wyscout') return <div>
    <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'flex-start',flexWrap:'wrap',marginBottom:14}}>
      ...
      <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}><SourceSwitch value={sourcePreference} available={availableSources} onChange={onSourceChange}/><button onClick={fetchTeams} style={{border:`1px solid ${GFC}`,background:'#fff',color:GFC,borderRadius:8,padding:'7px 12px',fontSize:10,fontWeight:800,cursor:'pointer'}}>🔄 Atualizar</button></div>
    </div>
      ...
      {visible.map(team=><Link key={team.team_name} href={`/ligas-v2/${slug}/times/${encodeURIComponent(team.team_name)}?source=${source}`} style={{textDecoration:'none'}}>
        <article style={{background:'#fff',border:'1px solid #dbe7f2',borderRadius:13,padding:15,height:'100%',boxSizing:'border-box',transition:'transform .15s,box-shadow .15s'}}
          ...
          <div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'flex-start',marginBottom:12}}>
            <div>
              <p style={{fontSize:14,fontWeight:900,color:GFC,lineHeight:1.2}}>{team.team_name}</p>
            ...
            <div style={{textAlign:'center',background:'#eaf4fd',borderRadius:10,padding:'6px 9px',minWidth:46}}>
              <p style={{fontSize:16,fontWeight:900,color:GFC}}>{fmt(team.profile_score,0)}</p>
