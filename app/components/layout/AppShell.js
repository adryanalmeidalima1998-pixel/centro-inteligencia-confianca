...
export default function AppShell({children}){
 const {data:session}=useSession(); const [open,setOpen]=useState(false); const [blocked,setBlocked]=useState(false)
 ...
 return <div className="min-h-screen bg-[#f4f7fb] flex">
   <OfflineBanner/>
   {open&&<div className="fixed inset-0 bg-slate-950/50 backdrop-blur-[2px] z-40 md:hidden" onClick={()=>setOpen(false)}/>} 
   <div className={`fixed md:sticky top-0 left-0 z-50 md:z-20 transform transition-transform duration-200 ${open?'translate-x-0':'-translate-x-full md:translate-x-0'}`}><Sidebar onClose={()=>setOpen(false)}/></div>
   <div className="flex-1 min-w-0">
    <header className="md:hidden sticky top-0 z-30 h-14 bg-white/95 backdrop-blur border-b border-slate-200 flex items-center px-4 gap-3"><button onClick={()=>setOpen(true)} className="w-9 h-9 rounded-xl border border-slate-200 grid place-items-center text-slate-600"><Menu size={18}/></button><img src="/confianca.svg" alt="Confiança" className="w-8 h-8"/><div><p className="text-[8px] font-black uppercase tracking-[.18em] text-[#0a66b7]">Confiança</p><p className="text-[10px] font-black">Centro de Inteligência</p></div></header>
