...
export default function RecruitmentPage() {
  ...
  const [market,setMarket]=useState([]); const [leagues,setLeagues]=useState([]); const [clubData,setClubData]=useState(null)
  ...
  useEffect(()=>{ Promise.all([
    fetch('/api/club-sportsbase').then(r=>r.json()), fetch('/api/ligas-v2/jogadores?limit=1').then(r=>r.json()),
  ]).then(([g,m])=>{setClubData(g);setLeagues(m.ligas||[])}).finally(()=>setLoading(false)); loadFoci() },[])
  ...
  return <ScoutingPage maxWidth={1540}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'285px minmax(0,1fr)',gap:14,alignItems:'start'}} className="scout-two-col">
      ...
      {!selected?<Panel><EmptyState icon="🎯" title="Selecione ou crie um foco" text="O sistema vai traduzir a necessidade em recorte de mercado, perfis e ações de scouting."/></Panel>:<div style={{display:'grid',gap:14}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:9}}><Kpi label="Mercado elegível" value={market.length} sub="após idade, posição e amostra" icon="🌐"/><Kpi label="No pipeline" value={pipeline.length} sub={`${pipeline.filter(c=>c.etapa==='Alvo prioritário').length} alvos prioritários`} icon="🧭" tone={C.purple}/><Kpi label="Modelo Confiança" value={clubData?.model?.identity?'Ativo':'Pendente'} sub={clubData?.model?.identity||'suba a planilha coletiva'} icon="🟢" tone={C.blue}/><Kpi label="Cobertura" value={`${pipeline.length?Math.round(pipeline.reduce((s,c)=>s+(c.fit_score||0),0)/pipeline.length):0}`} sub="score médio dos candidatos" icon="📈" tone={C.amber}/></div>
