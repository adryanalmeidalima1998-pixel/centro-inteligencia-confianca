...
function ranking(rows, metricName, { limit = 8, mode = 'total', minMinutes = 300 } = {}) {
  ...
  return eligibleRows(rows, col, minMinutes)
    .map(row => ({
      name:row.player, team:row.team, isClub:row.is_club, row,
...
export default function SerieCGoleirosPage() {
  ...
  const volumeEfficiency = useMemo(() => {
    ...
    return goalkeepers.map(row => ({ name:row.player, team:row.team, isClub:row.is_club, shots:toNumber(row.metrics?.[shotsOnTargetCol]), savePct:toNumber(row.metrics?.[saveCol]) })).filter(item => item.shots !== null && item.savePct !== null)
