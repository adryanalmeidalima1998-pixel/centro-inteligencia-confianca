...
function aggregateConfiancaMatchStats(rows) {
  ...
  for (const row of rows || []) {
    const home=isCurrentClubIdentity(row.home_team,row.home_code)
    const away=isCurrentClubIdentity(row.away_team,row.away_code)
...
function expectedPerformanceFromMatches(rows, clubStanding) {
  const timeline = []
  let xg = 0, xga = 0, goals = 0, goalsAgainst = 0
  for (const row of rows || []) {
    const clubHome = isCurrentClubIdentity(row.home_team, row.home_code)
    const clubAway = isCurrentClubIdentity(row.away_team, row.away_code)
    if (!clubHome && !clubAway) continue
    const ownMetrics = clubHome ? row.home_metrics : row.away_metrics
    const oppMetrics = clubHome ? row.away_metrics : row.home_metrics
    ...
    const gf = Number(clubHome ? row.home_score : row.away_score) || 0
    const ga = Number(clubHome ? row.away_score : row.home_score) || 0
    xg += matchXg
    xga += matchXga
    goals += gf
    goalsAgainst += ga
    timeline.push({
      ...
      opponent: clubHome ? row.away_team : row.home_team,
      xg: matchXg,
      xga: matchXga,
  ...
  if (timeline.length) {
    return {
      ...
      xPoints: Number.isFinite(Number(clubStanding?.xPoints)) ? Number(clubStanding.xPoints) : null,
      timeline,
    }
  }
  if (clubStanding && Number.isFinite(Number(clubStanding.xg)) && Number.isFinite(Number(clubStanding.xga))) {
    const sxg = Number(clubStanding.xg)
    const sxga = Number(clubStanding.xga)
    return {
      source: 'pdf', matches: Number(clubStanding.played) || null,
      xg: sxg, xga: sxga, xgDiff: sxg - sxga,
      xgPerMatch: clubStanding.played ? sxg / Number(clubStanding.played) : null,
      xgaPerMatch: clubStanding.played ? sxga / Number(clubStanding.played) : null,
      goals: Number(clubStanding.goalsFor) || 0,
      goalsAgainst: Number(clubStanding.goalsAgainst) || 0,
      goalsMinusXg: (Number(clubStanding.goalsFor) || 0) - sxg,
      goalsAgainstMinusXga: (Number(clubStanding.goalsAgainst) || 0) - sxga,
      xPoints: Number.isFinite(Number(clubStanding.xPoints)) ? Number(clubStanding.xPoints) : null,
      timeline: [],
    }
  }
...
export async function GET(request) {
  try {
    ...
    const [teamsRes, playersRes, gkRes, exclusionsRes] = await Promise.all([
      sql`SELECT team, is_club, metrics FROM serie_c_team_stats WHERE upload_id = ${currentUpload.id}`,
      sql`SELECT player, team, is_club, position, age, minutes, metrics FROM serie_c_player_stats WHERE upload_id = ${currentUpload.id}`,
      sql`SELECT player, team, is_club, age, minutes, metrics FROM serie_c_goalkeeper_stats WHERE upload_id = ${currentUpload.id}`,
      sql`
        SELECT player
    ...
    let previousTeams = []
    if (previousUpload) {
      const prevRes = await sql`SELECT team, is_club, metrics FROM serie_c_team_stats WHERE upload_id = ${previousUpload.id}`
    ...
    const clubStanding = standingsRows.find(row => normTeamName(row?.team).includes('CONFIANCA')) || null
    const seasonReport = standingsRes.rows[0]?.report_data && typeof standingsRes.rows[0].report_data === 'object'
      ...
      : {}
    const expectedPerformance = expectedPerformanceFromMatches(matchXgRes.rows, clubStanding)
    ...
    const seasonReportTimeline = reportHistoryRes.rows.map(snapshot => {
      ...
      const clubStandingRow = rows.find(row => normTeamName(row?.team).includes('CONFIANCA')) || null
      return {
    ...
    for (const u of sameSeasonUploads) {
      const teamRow = u.id === currentUpload.id
        ? teamsRes.rows.find(t => t.is_club)
        : (await sql`SELECT metrics FROM serie_c_team_stats WHERE upload_id = ${u.id} AND is_club = TRUE LIMIT 1`).rows[0]
      timeline.push({
        round: u.round,
        uploadDate: u.upload_date,
        clubPosition: u.club_position,
        clubPosition: u.club_position,
