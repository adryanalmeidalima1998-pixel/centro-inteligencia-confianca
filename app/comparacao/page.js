...
function Picker({label,mode,setMode,query,setQuery,options,selected,onSelect,clubAvailable=true}){
...
export default function ComparisonPage(){
  const [groups,setGroups]=useState(['AM']); const [league,setLeague]=useState(''); const [foot,setFoot]=useState(''); const [leagues,setLeagues]=useState([]); const [market,setMarket]=useState([]);const [clubData,setClubData]=useState(null);const [loading,setLoading]=useState(true)
  ...
  useEffect(()=>{
    ...
    Promise.all([fetch('/api/club-sportsbase').then(r=>r.json()),fetch('/api/ligas-v2/jogadores?limit=1').then(r=>r.json())]).then(([g,m])=>{setClubData(g);setLeagues(m.ligas||[])}).finally(()=>setLoading(false))
  },[])
  useEffect(()=>{if(!clubData||!selectedPositions)return;setLoading(true);const params=new URLSearchParams({posicao:selectedPositions,limit:'600',minMin:'0',ordem:'minutos',dir:'desc'});if(league)params.set('liga',league);if(foot)params.set('pe',foot);fetch(`/api/ligas-v2/jogadores?${params}`).then(r=>r.json()).then(json=>{const pool=json.jogadores||[];setMarket(enrichScoutingPool(pool,clubData.model||null,450));const search=new URLSearchParams(window.location.search);const wanted=search.get('player')||search.get('nome');const wantedTeam=search.get('equipa');if(wanted){const found=pool.find(p=>p.nome===wanted&&(!wantedTeam||p.equipa===wantedTeam));if(found)setRawA(current=>current||found)}}).finally(()=>setLoading(false))},[groupsKey,league,foot,clubData,selectedPositions])
  ...
  const clubPool=useMemo(()=>enrichScoutingPool((clubData?.players||[]).filter(p=>includesPosition(p,selectedPositions)&&matchesPlayerFoot(p,foot)),clubData?.model||null,0),[clubData,selectedPositions,foot])
  const comparisonPool=useMemo(()=>market.map(p=>{const copy={...p};delete copy._scouting;return copy}),[market])
  const enrichSelected=raw=>raw?buildScoutingPlayer(raw,comparisonPool.length?comparisonPool:[raw],clubData?.model||null,450):null
  const a=useMemo(()=>enrichSelected(rawA),[rawA,comparisonPool,clubData]);const b=useMemo(()=>enrichSelected(rawB),[rawB,comparisonPool,clubData])
  const optionsA=modeA==='confianca'?clubPool:market;const optionsB=modeB==='confianca'?clubPool:market
  ...
  if(loading&&!clubData)return <ScoutingPage><LoadingState/></ScoutingPage>
  return <ScoutingPage maxWidth={1460}>
    ...
    <div className="scout-two-col" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}><Picker label="Atleta A" mode={modeA} setMode={setModeA} query={queryA} setQuery={setQueryA} options={optionsA} selected={a} onSelect={setRawA} clubAvailable={clubPool.length>0}/><Picker label="Atleta B" mode={modeB} setMode={setModeB} query={queryB} setQuery={setQueryB} options={optionsB} selected={b} onSelect={setRawB} clubAvailable={clubPool.length>0}/></div>
