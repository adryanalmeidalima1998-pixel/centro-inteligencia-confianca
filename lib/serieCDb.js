...
export async function ensureSerieCTables() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS serie_c_club_matches (
      id           SERIAL PRIMARY KEY,
      season       VARCHAR(20) NOT NULL,
      competition  VARCHAR(120) NOT NULL DEFAULT 'Brasileiro Série C',
      match_date   DATE,
  ...
  try {
    await sql`
      ALTER TABLE serie_c_club_matches
      ADD CONSTRAINT serie_c_club_matches_unique UNIQUE (season, competition, match_date, opponent)
    `
  } catch (_) { /* já existe */ }
  try { await sql`CREATE INDEX IF NOT EXISTS idx_sc_matches_round ON serie_c_club_matches(season, competition, round)` } catch (_) {}
