...
export function createExecutivePdf(report) {
  ...
  const g = report.guarani?.summary || {}
  autoTable(doc, {
    startY: y,
    head: [['Jogos', 'Pontos', 'Aproveitamento', 'GP', 'GC', 'Saldo', 'Identidade']],
    body: [[g.games || 0, g.points || 0, `${number(g.performance, 1)}%`, g.goalsFor || 0, g.goalsAgainst || 0, g.goalDifference || 0, report.guarani?.model?.identity || 'Em construção']],
    styles: { fontSize: 8.2, cellPadding: 2.4 },
    headStyles: { fillColor: GREEN, textColor: 255 },
  })
  y = doc.lastAutoTable.finalY + 8
