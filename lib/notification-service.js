/**
 * Serviço central de notificações do CIC
 * POST { tipo, dados }
...
const RESEND_EMAIL = String(process.env.NOTIF_EMAIL || '').trim()
const WHATSAPP_PHONE = String(process.env.WHATSAPP_PHONE || '').replace(/\D/g, '')

async function sendEmail({ subject, html }) {
  const key = process.env.RESEND_API_KEY
  if (!key || !RESEND_EMAIL) return { skipped: true, reason: 'RESEND_API_KEY/NOTIF_EMAIL não configurados' }
...
async function sendWhatsApp(message) {
  ...
  if (!apiKey || !WHATSAPP_PHONE) return { skipped: true, reason: 'CALLMEBOT_API_KEY/WHATSAPP_PHONE não configurados' }
...
function emailWrap(conteudo) {
  ...
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden;border:1px solid #e2e8f0">
    <div style="background:#0a66b7;padding:18px 22px;display:flex;align-items:center;gap:10px">
      <div style="background:rgba(255,255,255,.15);width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:900;color:#fff;flex-shrink:0">ADC</div>
...
export async function dispatchNotification(tipo, dados) {
  try {
    const results = {}
    ...
    if (tipo === 'lista_final') {
      ...
      const { atletas } = dados
      if (!atletas?.length) return { skipped: true }
      ...
      const { jogos } = dados
      if (!jogos?.length) return { skipped: true }
    ...
    return { success: true, tipo, ...results }
  } catch (err) {
    throw err
  }
}
