...
import { evaluateClubMarketContext, CLUB_COMPETITIVE_CONTEXT, isOwnClub } from '@/data/club-market-context'
...
function prepareSnapshot(row, seasonYear) {
  ...
  for (const group of EVOLUTION_POSITION_ORDER) {
    ...
    ranked.forEach((player, index) => {
      ...
      const rank = index + 1
      const market = evaluateClubMarketContext({ ...player, _liga: row.slug }, row.slug)
      const current = {
        key,
...
function isClubContextEligible(player, marketScope = 'immediate') {
  const market = player?.market
  if (market?.ownClub || isOwnClub(player?.clube)) return false
...
function contextualExclusionReason(player, marketScope = 'immediate') {
  const market = player?.market
  if (market?.ownClub || isOwnClub(player?.clube)) return 'Atleta do próprio Confiança; excluído da prospecção externa.'
...
function buildLeagueEvolution(slug, snapshotsBySource, options) {
  ...
  const contextualPlayers = allCurrentEvolution.filter(player => isClubContextEligible(player, options.marketScope))
  const excludedContextPlayers = allCurrentEvolution
    .filter(player => !isClubContextEligible(player, options.marketScope))
    .map(player => ({
      key: player.key,
      nome: player.nome,
      clube: player.clube,
  ...
  return {
    ...
    marketPolicyVersion: CLUB_COMPETITIVE_CONTEXT.policyVersion,
