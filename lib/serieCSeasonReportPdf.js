...
function parseRankedGuarani(tokens, heading, valueCount, occurrence = 0) {
  let from = 0
  let headingIndex = -1
...
function parseGoalDynamics(tokens, heading) {
  ...
  let guaraniIndex = findExact(tokens, 'Confiança', totalHeader + 1)
  if (guaraniIndex < 0) return null

  const values = tokens.slice(guaraniIndex + 1, guaraniIndex + 19)
...
function parseTeamProfile(pages) {
  ...
  const goals = parseRankedGuarani(p6, 'Golo', 2)
  const conceded = parseRankedGuarani(p6, 'Golos sofridos', 2)
  const shots = parseRankedGuarani(p6, 'Remate', 3)
  const shotsAgainst = parseRankedGuarani(p6, 'Remates sofridos', 3)
  ...
  const losses = parseRankedGuarani(p7, 'Perdas', 7)
  const recoveries = parseRankedGuarani(p7, 'Recuperações', 7)
  if (losses) {
  ...
  const organization = parseRankedGuarani(p8, 'Organização', 10)
  const possession = parseRankedGuarani(p8, 'Posse de bola', 7)
  if (organization) {
  ...
  const finalThirdPasses = parseRankedGuarani(p9, 'Passes para terço final', 2)
  const deepReceptions = parseRankedGuarani(p9, 'Receção de passes em profundidade', 1)
  const progressivePasses = parseRankedGuarani(p9, 'Passes progressivos', 2)
  const deepPasses = parseRankedGuarani(p9, 'Passe para profundidade', 2)
  ...
  const crosses = parseRankedGuarani(p10, 'Cruzamento', 6)
  const dribbles = parseRankedGuarani(p10, 'Dribles', 2)
  const boxTouches = parseRankedGuarani(p10, 'Toques na área', 1)
  const foulsSuffered = parseRankedGuarani(p10, 'Faltas sofridas', 2)
  if (crosses) {
  ...
  const offensiveDuels = parseRankedGuarani(p11, 'Duelos ofensivos', 2)
  const defensiveDuels = parseRankedGuarani(p11, 'Duelos defensivos', 3)
  const aerialDuels = parseRankedGuarani(p11, 'Duelos aéreos', 2)
  const looseBallDuels = parseRankedGuarani(p11, 'Duelos bolas perdidas', 2)
  ...
  const interceptions = parseRankedGuarani(p12, 'Intercepção', 1)
  const ppda = parseRankedGuarani(p12, 'Intensidade de pressão (PPDA)', 1)
  const blockedShots = parseRankedGuarani(p12, 'Remates intercetados', 3)
  const fouls = parseRankedGuarani(p12, 'Falta', 3)
  ...
  const penalties = parseRankedGuarani(p13, 'Penáltis', 2)
  const directFreeKicks = parseRankedGuarani(p13, 'Livres directos', 1)
  if (penalties) {
...
function extractGuaraniLeaderMentions(pages) {
  const results = []
...
export function parseWyscoutSeasonReportPages(pages, standingsRows = []) {
  ...
  const guaraniStandings = (standingsRows || []).find(row => clean(row?.team).toLocaleLowerCase('pt-BR').includes('confianca')) || null

  return {
    version: 1,
    guarani: {
      standings: guaraniStandings,
    ...
    leaders: extractGuaraniLeaderMentions(pages),
