...
export function squadLeaders(clubPlayers, opts = {}) {
  return leadersFromCatalog(clubPlayers, PLAYER_CATALOG, opts)
}

export function goalkeeperLeaders(clubGoalkeepers, opts = {}) {
  return leadersFromCatalog(clubGoalkeepers, GK_CATALOG, opts)
}
...
export function collectiveReport(teams) {
  ...
  const rows = TEAM_METRICS.map(def => {
    const graded = competitionTeams
      .map(t => ({ team:t.team, is_club:t.is_club, value:teamVal(t, def.col) }))
      .filter(t => t.value !== null)
      .sort((a,b) => def.higher ? b.value - a.value : a.value - b.value)
    const total = graded.length
    const idx = graded.findIndex(t => t.is_club)
