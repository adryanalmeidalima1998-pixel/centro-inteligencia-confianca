...
export function isGuaraniMatch(match) {
  const home = normTeamName(match?.home_team || match?.homeTeam)
  const away = normTeamName(match?.away_team || match?.awayTeam)
  return home.includes('CONFIANCA') || away.includes('CONFIANCA') || match?.home_code === 'CON' || match?.away_code === 'CON'
}
