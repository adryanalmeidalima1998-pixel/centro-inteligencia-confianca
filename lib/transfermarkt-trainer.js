...
function parseCareerReadable(source = '') {
  ...
  for (let i = header + 1; i < lines.length; i++) {
    ...
    // O Transfermarkt/Jina frequentemente quebra a linha compacta em duas:
    //   | ASA
    //   Treinador | 25/26 (02/03/2026) | - | 25 | 1.84
    // Nesse formato o clube é uma linha markdown de apenas uma célula.
    if (cells.length === 1 && !/Treinador/i.test(cells[0]) && !/^(Time e cargo|Entrada|Saída do clube|Jogos|PPJ)$/i.test(cells[0])) {
      pendingClub = cells[0].replace(/^[-*]\s*/, '').trim()
      continue
    }
...
function parseCareer(html) {
  ...
  for (const row of extractRows(table)) {
    ...
    const rowText = all.join(' ').replace(/\s+/g, ' ').trim()
    if (/Time e cargo|Team and position/i.test(rowText) && /PPJ/i.test(rowText)) continue

    // Não assume mais que o clube está na primeira célula: algumas tabelas
    // trazem uma coluna separada para o escudo antes do nome do clube.
    const clubCell = cells
      .map((c, idx) => ({ idx, text: anchorTextByHref(c.html, /\/verein\//i) }))
      .find(x => x.text)
    const club = clubCell?.text || null
    if (!club) continue

    const role = rowText.match(/\b(Treinador(?:\s+interino)?|Auxiliar(?:\s+técnico)?|Assistente(?:\s+técnico)?|Manager)\b/i)?.[1] || 'Treinador'

    const dates = []
    for (const cell of cells) {
      const matches = [...String(cell.text || '').matchAll(/\d{2}\/\d{2}\/\d{4}/g)].map(m => m[0])
      for (const d of matches) if (!dates.includes(d)) dates.push(d)
    }
    const entrada = dates[0] || null
    let saida = dates[1] || null

    // Linha atual normalmente traz '-' no campo de saída.
    const afterClub = cells.slice((clubCell?.idx ?? 0) + 1)
    if (afterClub.some(c => /^[-–—]$/.test(String(c.text || '').trim()))) saida = null

    // PPJ: último número decimal plausível da linha compacta.
    let ppj = 0
    for (let i = cells.length - 1; i >= 0; i--) {
      const value = String(cells[i].text || '').trim()
      if (/^\d+[.,]\d{1,2}$/.test(value)) {
        const n = Number(value.replace(',', '.'))
        if (Number.isFinite(n) && n >= 0 && n <= 5) { ppj = n; break }
      }
    }

    // Jogos: prioriza a célula com link para desempenho; se não existir,
    // pega o inteiro imediatamente anterior ao PPJ na tabela compacta.
    let jogos = 0
    const linkedGame = cells
      .map(c => ({ text:String(c.text || '').trim(), html:c.html }))
      .find(c => /^\d+$/.test(c.text) && /leistungsdaten|station_id|verein_id|wettbewerb/i.test(c.html))
    if (linkedGame) jogos = Number(linkedGame.text) || 0
    if (!jogos) {
      for (let i = cells.length - 1; i >= 0; i--) {
        const value = String(cells[i].text || '').trim()
        if (/^\d+$/.test(value)) {
          const n = Number(value)
          if (n >= 0 && n < 1000) { jogos = n; break }
        }
      }
    }

    rows.push({ clube:club, cargo:role, entrada, saida, jogos, ppj })
  }

  return rows.filter(row => row.clube)
}
...
export function normalizeTransfermarktTrainerUrl(input) {
  ...
  return {
    ...
    stationsUrl: `${origin}/${nameSlug}/stationen/trainer/${id}/plus/1`,
...
export async function scrapeTransfermarktTrainer(inputUrl, options = {}) {
  ...
  const [profileResult, stationsResult, performanceResult] = await Promise.allSettled([
    fetchTransfermarktPage(urls.profileUrl),
    fetchTransfermarktPage(urls.stationsUrl),
  ...
  const stationsPage = stationsResult.status === 'fulfilled' ? stationsResult.value : { html:'', readable:'' }
  const performancePage = performanceResult.status === 'fulfilled' ? performanceResult.value : { html:'', readable:'' }
  const stationsWarning = stationsResult.status === 'rejected' ? stationsResult.reason?.message || 'Falha na página de trabalhos desenvolvidos.' : null
  ...
  const stationsHtml = stationsPage.html || ''
  const performanceHtml = performancePage.html || ''
  let profileReadable = profilePage.readable || ''
  let stationsReadable = stationsPage.readable || ''
  let performanceReadable = performancePage.readable || ''
  ...
  // Carreira: tenta primeiro o quadro compacto do perfil e depois a página
  // específica de Trabalhos desenvolvidos. Isso cobre perfis como Itamar
  // Schulle, nos quais o Transfermarkt quebra clube e dados em linhas distintas.
  let carreira = parseCareer(profileHtml)
  if (!carreira.length) carreira = parseCareerReadable(profileReadable)
  if (!carreira.length) carreira = parseCareerReadable(stationsReadable)

  // Se a requisição direta trouxe HTML suficiente, mas a tabela não pôde ser
  // estruturada, força uma leitura markdown apenas para a carreira.
  if (!carreira.length) {
    const retries = await Promise.allSettled([fetchReader(urls.profileUrl), fetchReader(urls.stationsUrl)])
    if (retries[0].status === 'fulfilled') {
      profileReadable = retries[0].value
      carreira = parseCareerReadable(profileReadable)
    }
    if (!carreira.length && retries[1].status === 'fulfilled') {
      stationsReadable = retries[1].value
      carreira = parseCareerReadable(stationsReadable)
    }
  }

  let jogosRaw = parseMatches(performanceHtml)
  if (!jogosRaw.length) jogosRaw = parseMatchesReadable(performanceReadable)
  if (!jogosRaw.length && performancePage.source !== 'reader') {
    try {
      performanceReadable = await fetchReader(urls.performanceUrl)
      jogosRaw = parseMatchesReadable(performanceReadable)
    } catch {}
  }
  let ai = null
  let aiWarning = null
  ...
  return {
    ...
    aviso_ia: aiWarning,
    aviso_coleta: [stationsWarning, performanceWarning].filter(Boolean).join(' | ') || null
  }
}
