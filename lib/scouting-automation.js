...
import {
  evaluateGuaraniMarketContext,
  getGuaraniLeagueMarketPolicy,
  marketContextForDashboard,
  summarizeGuaraniMarket,
} from '@/data/guarani-market-context'
...
function contextualizeLeague(league) {
  const policy = getGuaraniLeagueMarketPolicy(league.slug)
  const data = (league.data || []).map(player => {
    const contextualPlayer = { ...player, _liga: league.slug, _source: league.source }
    return {
      ...contextualPlayer,
      _market: evaluateGuaraniMarketContext(contextualPlayer, league.slug),
    }
  })
  const summary = summarizeGuaraniMarket(data)
  return {
    ...league,
    market: {
      band: policy.band,
...
function dedupeMarketPlayers(pool = []) {
  ...
  for (const player of pool) {
    ...
    const playerContext = player._market || evaluateGuaraniMarketContext(player, player._liga)
    const currentContext = current?._market || (current ? evaluateGuaraniMarketContext(current, current._liga) : null)
...
function buildOpportunities(pool, model, focuses, previousPayload) {
  ...
  const opportunities = enriched.map(player => {
    const scout = player._scouting || {}
    const market = player._market || evaluateGuaraniMarketContext(player, player._liga)
...
function buildNeeds(guarani, focuses, pipeline, opportunities, enrichedGuarani) {
  ...
  const counts = (guarani.players || []).reduce((acc, player) => {
    const group = positionGroup(player.posicao)
    if (group) acc[group] = (acc[group] || 0) + 1
    return acc
  }, {})
  const modelNeeds = [...(guarani.model?.needs || guarani.summary?.model?.needs || [])]
  ...
  for (const group of POSITION_GROUPS) {
    const list = enrichedGuarani.filter(player => positionGroup(player.posicao) === group)
...
function buildTimeline(guarani, session) {
  const history = Array.isArray(session?.historico) ? session.historico : []
  return (guarani.games || []).map(game => {
...
function buildExecutiveSummary(guarani, opportunities, needs, alerts, marketContext) {
  const model = guarani.model || guarani.summary?.model || {}
  ...
  const sentences = []
  if (guarani.summary?.games) sentences.push(`O Confiança soma ${guarani.summary.points || 0} pontos em ${guarani.summary.games} partidas, com ${round(guarani.summary.performance, 1)}% de aproveitamento, em um planejamento orientado ao acesso à Série C.`)
...
function buildAlerts({ coverage, guarani, focuses, pipeline, monitoring, finalRows, preferred, observations, opportunities, previousPayload, marketPool }) {
  const alerts = []
  ...
  if (!guarani.uploads?.team || !guarani.uploads?.players) alerts.push(makeAlert('CONFIANÇA', 'warning', 'Base do Confiança incompleta', 'A planilha coletiva ou individual Sportsbase ainda não foi carregada.', '/elenco', 'guarani-upload'))
  ...
  for (const candidate of pipeline) {
    ...
    const candidateMarket = evaluateGuaraniMarketContext({
...
function buildActions({ coverage, focuses, pipeline, monitoring, finalRows, observations, alerts }) {
  ...
  const viabilityReview = pipeline.filter(item => !evaluateGuaraniMarketContext({
...
function healthSummary(coverage, guarani) {
  ...
  if (!guarani.uploads?.team) issues.push({ code: 'guarani-team', severity: 'warning', league: 'Confiança', message: 'Planilha coletiva ausente.' })
  if (!guarani.uploads?.players) issues.push({ code: 'guarani-players', severity: 'warning', league: 'Confiança', message: 'Planilha individual ausente.' })
...
async function loadOperationalData() {
  const [uploads, guarani, focuses, pipeline, monitoring, finalRows, preferred, observations, sessionRows, importLogs] = await Promise.all([
    latestUploads(),
    getGuaraniSportsbase().catch(() => ({ games: [], players: [], summary: {}, model: null, uploads: {} })),
    ...
    safeRows(() => sql`SELECT payload FROM elenco_session WHERE session_key = 'guarani_elenco_2026' LIMIT 1`),
  ...
  return { uploads, guarani, focuses, pipeline, monitoring, finalRows, preferred, observations, session, importLogs }
}
...
export async function runScoutingAutomation(options = {}) {
  ...
  try {
    ...
    const marketSummary = summarizeGuaraniMarket(marketPool)
    ...
    const model = operational.guarani.model || operational.guarani.summary?.model || null
    const { opportunities, enriched } = buildOpportunities(actionablePool, model, operational.focuses, previous?.payload)
    const guaraniByGroup = operational.guarani.players || []
    const enrichedGuarani = guaraniByGroup.length
      ? enrichScoutingPool(guaraniByGroup, model, Math.max(270, getSuggestedMinimumMinutes(guaraniByGroup)))
      : []
    const needs = buildNeeds(operational.guarani, operational.focuses, operational.pipeline, opportunities, enrichedGuarani)
    const severityOrder = { critical: 0, warning: 1, ok: 2 }
    const alerts = buildAlerts({
      coverage, guarani: operational.guarani, focuses: operational.focuses, pipeline: operational.pipeline,
      ...
      .slice(0, 80)
    const health = healthSummary(coverage, operational.guarani)
    ...
    const timeline = buildTimeline(operational.guarani, operational.session)
    ...
    const payload = {
      ...
      topMetrics,
      guarani: {
        summary: operational.guarani.summary || {},
        model,
        uploads: operational.guarani.uploads || {},
        timeline,
        depth: operational.guarani.summary?.groups || {},
      },
      executiveSummary: '',
    ...
    payload.executiveSummary = buildExecutiveSummary(operational.guarani, opportunities, needs, alerts, payload.marketContext)
...
export async function getAutomationDashboard({ refreshIfMissing = true, maxAgeMinutes = 180 } = {}) {
  ...
  return { ...snapshot.payload, snapshotId: snapshot.id, snapshotCreatedAt: snapshot.created_at }
}
