import { sanitizeCoachReport } from './treinador-report-sanitizer'
...
14) REGRA DE EVIDÊNCIA: cada frase afirmativa do relatório precisa estar sustentada por um fato, padrão recorrente, texto fornecido, jogo importado ou dado factual disponível no input. Se não houver suporte, NÃO escreva a frase.
15) Ausência de informação NÃO é fraqueza e NÃO é ponto de melhoria. Nunca crie itens como "comportamento não documentado", "não há informação", "precisa validar" ou equivalentes. Pontos de melhoria só podem descrever limitações efetivamente observadas/sustentadas.
16) Em perfis_jogadores, retorne SOMENTE posições para as quais há exigência concreta de perfil sustentada pelo material. Não é obrigatório preencher todas as posições; use [] quando nenhuma exigência estiver sustentada.
17) Em adaptabilidade, inclua SOMENTE critérios que possam ser avaliados com suporte factual. Não use nota 0 para representar falta de informação.
18) Antes de responder, faça uma checagem interna: se uma conclusão não puder ser ligada a uma informação concreta do material, remova-a.`
  ...
  try {
    return sanitizeCoachReport(await responsesJson({ instructions, input, schema:REPORT_SCHEMA, schemaName:'coach_scouting_report', web:true, timeoutMs:52000 }))
  } catch (webErr) {
    ...
    return sanitizeCoachReport(await responsesJson({ instructions:fallbackInstructions, input, schema:REPORT_SCHEMA, schemaName:'coach_scouting_report_no_web', web:false, timeoutMs:45000 }))
  }
}
...
17) REGRA DE EVIDÊNCIA: só preencha um campo quando houver conteúdo concreto no material, no histórico factual ou no relatório anterior que sustente a conclusão.
18) Ausência de informação nunca deve virar ponto de melhoria. Pontos de melhoria exigem comportamento negativo, risco recorrente, resultado problemático ou limitação explicitamente sustentada.
19) perfis_jogadores: retorne apenas posições cuja exigência de perfil esteja sustentada. Não preencha posições por convenção tática ou suposição.
20) adaptabilidade: avalie apenas critérios sustentados por fatos. Não atribua estrelas quando o material não permitir avaliação.
21) Nunca use "inferência" para completar lacunas. Se não está sustentado, deixe de fora.`
  ...
  return sanitizeCoachReport(await responsesJson({ instructions, input, schema:TEXT_SOURCE_SCHEMA, schemaName:'coach_text_source_analysis', web:false, timeoutMs:36000, maxOutputTokens:4800 }))
}
...
13) REGRA DE EVIDÊNCIA: toda conclusão deve estar sustentada pelos jogos importados, por histórico factual ou por conteúdo válido já existente no relatório. Não complete lacunas por suposição.
14) Ausência de informação não é ponto de melhoria. Só inclua ponto de melhoria quando houver padrão negativo ou limitação sustentada por partidas/resultados/histórico.
15) Se um aspecto não estiver sustentado, preserve um texto anterior válido ou deixe o campo vazio. Nunca escreva avisos sobre falta de evidência.
16) Em adaptabilidade, só mantenha critérios que possuam justificativa factual; não use nota 0 como marcador de desconhecimento.`
...
${JSON.stringify(compactWyscout)}`
  return sanitizeCoachReport(await responsesJson({instructions,input,schema:WYSCOUT_ANALYSIS_SCHEMA,schemaName:'coach_wyscout_analysis',web:false,timeoutMs:42000,maxOutputTokens:5200}))
}
