...
export const CLUB_TEAM_COL_MAP = {
...
export function parseClubTeamRows(rows = []) {
  const games = []
  for (const row of rows) {
    ...
    for (const [header, key] of Object.entries(CLUB_TEAM_COL_MAP)) {
...
export function parseClubPlayerRows(rows = []) {
  return rows
    .map(row => parseSportsbaseRow({ ...row, Time: row?.Time || 'Confiança' }))
    .filter(player => player.nome && (Number(player.minutos) || 0) > 0)
    .map(player => ({ ...player, equipa: 'Confiança', equipa_periodo: 'Confiança', _fonte: 'sportsbase-confianca' }))
}
...
export function deriveClubModel(games = []) {
...
export function summarizeClubDataset(games = [], players = []) {
  ...
  const model = deriveClubModel(games)
