 ...
 */
export const GUARANI_COMPETITIVE_CONTEXT = {
  club: 'Confiança',
...
export const GUARANI_OWN_CLUB_ALIASES = [
  'confianca',
  'ad confianca',
...
export function isGuaraniOwnClub(value) {
  const normalized = normalizeMarketText(value)
  if (!normalized) return false
  if (GUARANI_OWN_CLUB_ALIASES.includes(normalized)) return true
...
export function getGuaraniLeagueMarketPolicy(slug) {
...
export function evaluateGuaraniMarketContext(player = {}, leagueSlug = '') {
  const policy = getGuaraniLeagueMarketPolicy(leagueSlug || player._liga || player.liga)
  ...
  const ownClub = isGuaraniOwnClub(currentClub)
  ...
  return {
    policyVersion: GUARANI_COMPETITIVE_CONTEXT.policyVersion,
    actionable,
...
export function summarizeGuaraniMarket(players = []) {
  const summary = {
  ...
  for (const player of players) {
    const context = player?._market || evaluateGuaraniMarketContext(player, player?._liga || player?.liga)
